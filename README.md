...
# Current Updates and Notes - updated 03-26-2026
- **Section heading keywords are now fully user-configurable** — the default keywords (`chapter`, `section`, `part`, `prologue`, `epilogue`, etc.) are pre-filled as editable pills in the Generate tab. Any keyword can be removed, new ones can be added, and changes immediately re-analyze the text. The incoming keyword list is now treated as the complete set rather than an additive override, so removing a default actually removes it.
- Added **Custom Chapter Export** to the library — a **✦ Select** button appears on any library item with multiple chapters. Click it to enter selection mode, toggle individual chapter pills (they highlight green when selected), optionally name the output file, then click **Export Selected** to merge only those chapters into a new downloadable audio file. Ideal for splitting long audiobooks into custom parts.
- **Bug fix**: Pausing and resuming a job no longer produces a duplicate tail in the full-story audio file. Previously, chapters processed before the pause were being appended to `all_full_story_chunks` a second time on resume, causing chapters to repeat in the merged output. The resume path now correctly skips already-restored chapter indices when extending the chunk list.
- **Bug fix**: **Repair** and **Rebuild All** in the library no longer replace real chapter titles with generic `"Chapter 1"`, `"Chapter 2"` names. The rebuild logic now reads chapter titles from `metadata.json` as the authoritative source (with automatic 1-based → 0-based index correction) rather than relying solely on the manifest, which could be missing or have incomplete titles.
